...
3. 使用命令 `bash hpc_env_check.sh` 运行环境检测脚本，了解登录节点的环境情况。
4. 正常来讲是没有登录节点是没有配置 conda 的。使用命令 `bash install_conda.sh` 来安装 conda。
...
5. 使用快捷键「Ctrl+J」打开终端窗口，然后使用 `cd` 命令跳转到本文件所在目录，然后使用命令 `bash gpu_env_check.sh` 来检测环境 GPU。
